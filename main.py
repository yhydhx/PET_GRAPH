...
# parameters
binary = True # true for binary classification; false for 4-classification
...
def train(train_data, train_labels, test_data, test_labels, L, lmax,A):
    ...
    torch.save(state, saved_path+"_model.pkl")
    ...
    for epoch in range(1000):  # loop over the dataset multiple times
        ...
        A,L,lmax = update_graph(A)
        trans_count = 0
        disazster_flag = 0
        while L[0].shape != layer1:

            A,L,lmax = update_graph(A)
            print(L[0].shape, layer1)
            trans_count += 1
            #disazster mechanize
            if trans_count == 100:
                A = old_A
                ...
                disazster_flag = 1
                break
        #train several epoch first 
        NUM_TRAIN_EPOCH = 10
        for m in range(NUM_TRAIN_EPOCH):
            # reshuffle
            indices.extend(np.random.permutation(train_size))  # rand permutation
            # extract batches
            running_loss = 0.0
        ...
        if flag == False and last + 150 < epoch+200:
            flag = True
        ...
        #keep the graph or not based on the result
        #if the accury is lower than previous one , load the previous model otherwise, save it.  
        if acc_test < old_acc:
            L = old_L
            lmax = old_Lmax
            A = old_A
            checkpoint = torch.load(saved_path+"_model.pkl")
    ...
    np.save("graph_A.npy",A.toarray())
    np.save(saved_path+"_graph_A.npy",old_A.toarray())
    print("step1 acc: ",step1_acc)
    print("step2 acc: ",old_acc)
    '''
    nwe graph ended
    '''    
...
def main():
    ...
    train(train_data, train_label, test_data, test_label, L, lmax,A)
