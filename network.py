...
class Graph_ConvNet_LeNet5(nn.Module):
    ...
    def __init__(self, net_parameters):
        ...
        # parameters
        D, CL1_F, CL1_K, CL2_F, CL2_K, CL3_F, CL3_K, FC1_F, FC2_F = net_parameters
        FC1Fin = CL2_F * (D // 16)
    ...
    def forward(self, x, d, L, lmax):
        ...
        # # graph CL3
        # x = self.graph_conv_cheby(x, self.cl3, L[4], lmax[4], self.CL3_F, self.CL3_K)
        # x = F.leaky_relu(x)
        # x = self.graph_max_pool(x, 4)
    ...
    def evaluation(self, y_predicted, test_l):
        ...
        return 100.0 * float((class_predicted == test_l).sum()) / float(y_predicted.size(0))

    def feature_importances(self, L):
        weight = list(self.cl1.parameters())
        w, b = weight
        w = w.cpu().data.numpy()
        b = b.cpu().data.numpy()
        L0 = np.eye(L[0].shape[0])
        L1 = L[0].toarray()
        L = L1
        ans = L0 * w[0, 0] + L0 * b[0] + L1 * w[0, 1] + L0 * b[0]
        for k in range(2, 25):
            L2 = 2 * np.matmul(L, L1) - L0
            ans += L2 * w[0, k] + L0 * b[0]
            L0, L1 = L1, L2
        ...
        im = ax.imshow(ans, cmap=plt.cm.hot_r)
        #增加右侧的颜色刻度条
        plt.colorbar(im)
        #增加标题
        plt.title("This is the feature map")
        #show
        plt.show()
